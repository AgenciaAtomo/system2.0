...
do $test$ declare actor uuid; org uuid; account uuid; cat uuid; child uuid; center uuid; key uuid=gen_random_uuid(); n integer; body jsonb; begin
 ...
 select count(*) into n from public.audit_logs where entity_id=account::text and domain='financial'; if n<>1 then raise exception 'audit or retry duplicated'; end if;
 execute 'reset role';
...
rollback; select 'PASS: catalogs, exact money, idempotency, mismatched retry, hierarchy/cycle, archive, escaped search, audit, viewer denied; rolled back' as result;
