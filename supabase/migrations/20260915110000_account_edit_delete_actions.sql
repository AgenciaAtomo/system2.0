...
begin
 ...
 if kind='account' then
  if operation='archive' then
   update public.contas_bancarias set ativa=false,deletada_em=coalesce(deletada_em,now()),atualizada_em=now() where id=target and empresa_id=org returning id into result;
  else
