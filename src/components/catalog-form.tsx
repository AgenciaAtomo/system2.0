...
export function CatalogForm({kind,requestId,record,parents}:Props){
 ...
 {kind==="account"&&!record&&<><label className="text-sm font-medium">Tipo<select name="type" className={style} defaultValue="bancaria"><option value="bancaria">Conta bancária</option><option value="carteira">Carteira</option><option value="caixa">Caixa</option><option value="marketplace">Marketplace</option><option value="digital">Conta digital</option></select></label><label className="text-sm font-medium">Saldo inicial (R$)<input name="opening_balance" inputMode="decimal" defaultValue="0,00" required className={style}/></label><p className="m-0 text-xs text-zinc-500">O saldo inicial é informado uma única vez. Moeda: real brasileiro.</p></>}
...
export function ArchiveCatalog({kind,record,requestId}:{kind:CatalogKind;record:CatalogRow;requestId:string}){
 const [state,action,pending]=useActionState(saveCatalog,{message:""});
 return <form action={action} onSubmit={event=>{if(!window.confirm("Arquivar "+record.name+"? O histórico será preservado."))event.preventDefault();}}>
 ...
 <Button variant="outline" size="sm" disabled={pending}>Arquivar</Button>{state.message&&<p role="alert" className="mt-2 text-sm text-red-700">{state.message}</p>}
 </form>;
}
