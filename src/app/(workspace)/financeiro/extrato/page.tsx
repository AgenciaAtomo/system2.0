...
const emptyWorkspace=(page:number)=>workspace.parse({pending_total:0,page,items:[],imports:[]});
export default async function BankStatementPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 let result=emptyWorkspace(page),activeAccounts:{id:string;name:string}[]=[],activeCategories:{id:string;name:string;type:string}[]=[],canWrite=false,setupPending=false,loadIssue="";
 try{
  ...
  setupPending=!!data.error;
  if(data.data&&!setupPending)result=workspace.parse(data.data);
  if(accounts.error||categories.error||roles.error)loadIssue="Não consegui carregar contas, categorias ou permissões. Atualize a página depois de confirmar o acesso financeiro.";
  else{
   activeAccounts=(accounts.data??[]).filter(a=>a.ativa&&!a.deletada_em&&a.moeda==="BRL").map(a=>({id:a.id,name:a.nome}));
   activeCategories=(categories.data??[]).map(c=>({id:c.id,name:c.nome,type:c.tipo??"ambos"}));
   const roleIds=(roles.data??[]).map(r=>r.role_id);
   const grants=roleIds.length?await client.from("role_permissions").select("permission_id").in("role_id",roleIds).eq("permission_id","financial.write").limit(1):{data:[],error:null};
   if(grants.error)loadIssue="Não consegui verificar permissão de escrita. A consulta continua disponível, mas o envio ficou bloqueado.";
   canWrite=!!grants.data?.length;
  }
 }catch{
  setupPending=true;
  loadIssue="A área de extrato abriu em modo seguro porque uma consulta do banco falhou. Aplique a migration do leitor de extrato e atualize a tela.";
 }
 const canUseStatement=canWrite&&!setupPending&&!loadIssue;
 ...
 return <main className="content finance-content">
  ...
  {loadIssue&&<p className="notice" role="alert">{loadIssue}</p>}
