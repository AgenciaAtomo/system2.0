...
export default async function BankStatementPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 if(data.error||accounts.error||categories.error||roles.error)throw new Error("Não foi possível carregar o leitor de extrato.");
 ...
 const result=workspace.parse(data.data);
 ...
 return <main className="content finance-content">
  ...
  <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_360px]">
   <section className="min-w-0 rounded-xl border border-zinc-200 bg-white p-5 sm:p-6">
    ...
    <BankStatementDrafts drafts={result.items} categories={activeCategories} canWrite={canWrite}/>
   ...
   <aside className="h-fit rounded-xl border border-zinc-200 bg-white p-5 sm:p-6">
    ...
    {canWrite?<BankStatementUpload accounts={activeAccounts}/>:<p className="text-sm text-zinc-500">Seu perfil permite consultar os extratos processados.</p>}
    {(!activeAccounts.length||!activeCategories.length)&&canWrite&&<p className="mt-3 text-sm text-zinc-500">Cadastre uma conta e categorias antes de importar.</p>}
   </aside>
  </div>
