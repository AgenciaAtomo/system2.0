...
export default async function Finance({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <main className="content finance-content"><p className="eyebrow">FINANCEIRO</p><h1>Organize as bases da operação</h1><p className="text-zinc-500">Contas, categorias e centros de custo para seus lançamentos.</p><FinanceNav active={kind}/>{params.saved&&savedMessages[params.saved]&&<p className="notice" role="status">{savedMessages[params.saved]}</p>}<div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]"><section className="min-w-0 rounded-xl border border-zinc-200 bg-white p-6"><div className="mb-5 flex flex-wrap items-center justify-between gap-4"><h2 className="mb-0">{labels[kind]}</h2><span className="text-sm text-zinc-500">{result.total} registros</span></div><form className="mb-5 flex gap-2"><input type="hidden" name="kind" value={kind}/><input aria-label="Buscar por nome" name="q" defaultValue={search} maxLength={100} placeholder="Buscar por nome" className="min-w-0 flex-1 rounded-lg border border-zinc-300 px-3 text-sm"/><Button variant="outline">Buscar</Button></form>{!result.items.length?<p className="py-10 text-center text-zinc-500">{search?"Nenhum resultado para esta busca.":"Nenhum cadastro nesta lista. Use o formulário para começar."}</p>:<div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-zinc-200 text-zinc-500"><tr><th className="pb-3 font-medium">Nome · A-Z</th><th className="pb-3 font-medium">{kind==="account"?"Saldo inicial":kind==="category"?"Tipo":"Situação"}</th><th className="pb-3 font-medium">Editar/excluir</th></tr></thead><tbody className="divide-y divide-zinc-100">{result.items.map(row=><tr key={row.id}><td className="py-4 pr-4"><strong className="font-medium">{row.name}</strong><small className="mt-1 block text-zinc-500">{!row.active?(kind==="account"?"Excluída":"Arquivado"):kind==="account"?types[row.type??""]??row.type:row.parent_id?"Subcategoria":"Ativo"}</small></td><td className="py-4 pr-4 whitespace-nowrap">{kind==="account"?formatMoney(row.opening_balance??"0"):kind==="category"?types[row.type??""]:row.active?"Ativo":"Arquivado"}</td><td className="py-4">{canWrite&&row.active?<div className="flex flex-wrap gap-2"><Button asChild variant="outline" size="sm"><Link href={href(page)+"&edit="+row.id}><Pencil size={14} aria-hidden="true"/>Editar</Link></Button><ArchiveCatalog kind={kind} record={row} requestId={crypto.randomUUID()}/></div>:<span className="text-zinc-400">—</span>}</td></tr>)}</tbody></table></div>}<div className="mt-6 flex items-center justify-between text-sm"><span>Página {page} · 25 por página</span><div className="flex gap-3">{page>1&&<Link href={href(page-1)}>Anterior</Link>}{page*25<result.total&&<Link href={href(page+1)}>Próxima</Link>}</div></div></section>{canWrite&&<aside className="h-fit rounded-xl border border-zinc-200 bg-white p-6"><h2>{edit?"Editar cadastro":"Novo cadastro"}</h2><CatalogForm key={kind+":"+edit?.id} kind={kind} requestId={crypto.randomUUID()} record={edit} parents={parents.data??[]}/>{edit&&<Link className="mt-4 block text-sm underline" href={href(page)}>Cancelar edição</Link>}</aside>}</div><p className="mt-6 text-sm text-zinc-500">Registre entradas e saídas em Lançamentos. Vendas e compras vinculadas alimentam o financeiro automaticamente.</p></main>;
}
